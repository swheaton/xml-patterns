...
{
	return new Element(dynamic_cast<dom::Document *>(getAdaptee())->createElement(tagName));
}

XERCES::DOMText * adapter::Document::createTextNode(const XERCES::XMLCh * data)
{
	return new Text(dynamic_cast<dom::Document *>(getAdaptee())->createTextNode(data));
}

XERCES::DOMAttr * adapter::Document::createAttribute(const XERCES::XMLCh * name)
{
	return new Attr(dynamic_cast<dom::Document *>(getAdaptee())->createAttribute(name));
}

XERCES::DOMElement * adapter::Document::getDocumentElement() const
{
	return new Element(dynamic_cast<dom::Document *>(getAdaptee())->getDocumentElement());
}
